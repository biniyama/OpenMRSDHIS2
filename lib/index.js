  ...
  app.post('/send', async (req, res) => {
    ...
    try {
      ...
      if(session_detail.authenticated) {
        ...
        let location_data
        try{
          location_data = await fetch(mediatorConfig.config.OpenMRSURL + location , {
            method: "GET",
            headers: {
        ...
        for(var loc of location_detail.results) {
          ...
          console.log("UUID: " + loc.uuid + ", Name: " + loc.display + ", Number of visits: " + total_count)
        }

        res.send(location_detail)  
      }
      //console.log(session_data)
